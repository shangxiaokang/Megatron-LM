...
#===============================================================================
export MODEL_VARIANT=${MODEL_VARIANT:-35b_a3b_light}
...
WANDB_RUN_ID_FILE="${CHECKPOINT_STORE_PATH}/wandb_run_id.txt"
mkdir -p "${CHECKPOINT_STORE_PATH}"
if [[ -s "\${WANDB_RUN_ID_FILE}" ]]; then
    WANDB_RUN_ID="\$(tr -d '[:space:]' < "\${WANDB_RUN_ID_FILE}")"
    echo "[INFO] Reusing WANDB_RUN_ID=\${WANDB_RUN_ID}"
else
    WANDB_RUN_ID="\$(python3 -c 'import uuid; print(uuid.uuid4().hex)' 2>/dev/null || python -c 'import uuid; print(uuid.uuid4().hex)')"
    if [[ -z "\${WANDB_RUN_ID}" ]]; then
        echo "[ERROR] Failed to generate WANDB_RUN_ID."
        exit 1
    fi
    echo "\${WANDB_RUN_ID}" > "\${WANDB_RUN_ID_FILE}"
    echo "[INFO] Created WANDB_RUN_ID=\${WANDB_RUN_ID}"
fi
export WANDB_RUN_ID
